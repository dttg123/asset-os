...
const context=vm.createContext({console,Date,Set,ArrayBuffer,Uint8Array,DataView,TextEncoder,TextDecoder,JSON,Math,clone,normalizeState:clone,APP_ENV:'live'});
...
for(let schema=4;schema<=20;schema++){
 context.__payload={format:'asset-os-backup-v1',schemaVersion:schema,appVersion:schema===4?'v0.1':'v0.5',environment:'live',exportedAt:'2026-09-02T00:00:00.000Z',data:{accounts:[],pension:{},integrated:{ledger:[]}}};
...
context.__payload={format:'asset-os-backup-v1',schemaVersion:20,appVersion:'v0.5',environment:'qa',data:{accounts:[],pension:{},integrated:{ledger:[]}}};
assert.throws(()=>vm.runInContext('validateBackupPayload(__payload)',context),/QA 백업은 운영 화면에 복원할 수 없습니다/);
