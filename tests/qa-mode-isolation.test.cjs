...
assert.equal(run('QA_MODE'),true);assert.equal(run('APP_VERSION'),'v0.6.1');assert.equal(run('KEY'),'asset-os-qa-v0.5');assert.equal(run('localYmd()'),'2060-12-31');
...
assert.equal(run('state.system.qaDataset.version'),'v0.6.1');
...
assert.equal(run('homeSample().contributed'),2416674,'홈 이번 달 납입은 ISA·연금저축·IRP를 모두 합산해야 한다');
