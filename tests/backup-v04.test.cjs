...
const context=vm.createContext({console,Date,Set,Map,Promise,APP_VERSION:'v0.6.12',APP_ENV:'live'});
vm.runInContext(fs.readFileSync(path.join(__dirname,'..','backup-v04.js'),'utf8'),context,{filename:'backup-v04.js'});

assert.equal(vm.runInContext("assetManagedBackupName({date:new Date(2026,8,2),version:'v0.6.12'})",context),'AssetOS_260902_v0.6.12.zip');
...
assert.ok(names.includes('AssetOS_260801_v0.6.12.zip'));
assert.ok(names.includes('AssetOS_260703_v0.6.12.zip'));
assert.ok(!names.some(name=>name.endsWith('_pre.zip')||name.endsWith('_keep.zip')));
const qaContext=vm.createContext({console,Date,Set,Map,Promise,APP_VERSION:'v0.6.12',APP_ENV:'qa'});
vm.runInContext(fs.readFileSync(path.join(__dirname,'..','backup-v04.js'),'utf8'),qaContext,{filename:'backup-v04.js'});
assert.equal(vm.runInContext("assetManagedBackupName({date:new Date(2026,8,2)})",qaContext),'AssetOS_QA_260902_v0.6.12.zip');
assert.equal(vm.runInContext('ASSET_BACKUP_DB',qaContext),'asset-os-backup-v05-qa');
console.log('v0.6.12 backup retention tests: PASS');
