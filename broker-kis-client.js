...
const brokerKisClient=(()=>{
 let config={projectUrl:'',publishableKey:'',functionName:'kis-read'};
 ...
 function configure(input={}){
  const projectUrl=cleanUrl(input.projectUrl),publishableKey=cleanText(input.publishableKey,300),functionName=cleanText(input.functionName||'kis-read',80);
  ...
  config={projectUrl,publishableKey,functionName};return{ok:true}
 }
 ...
 async function requestOtp(email){
  ...
  const result=await jsonRequest(`${config.projectUrl}/auth/v1/otp`,{method:'POST',headers:publicHeaders(),body:JSON.stringify({email:value,create_user:false})});return result.ok?{ok:true}:{ok:false,status:result.status,error:result.error}
 }
 ...
 return{configure,authState,signOut,requestOtp,verifyOtp,invoke,sync}
})();
