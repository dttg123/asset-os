...
function policyForYear(kind,year){const group=state.policies[kind];if(!group?.versions?.length)return policy(kind);const y=Number(year)||businessYear(),end=`${y}-12-31`,eligible=group.versions.filter(v=>!v.effectiveFrom||String(v.effectiveFrom)<=end).sort((a,b)=>String(a.effectiveFrom||'').localeCompare(String(b.effectiveFrom||'')));return eligible.at(-1)||group.versions[0]||{}}
