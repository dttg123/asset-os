...
function qaBuildThirtyFiveYearState(){
 ...
 normalized.system.qaDataset={generatedAt:new Date().toISOString(),range:'2026-2060',months:QA_MONTHS,scenario:'real-user-35-years',homeLoanRemaining:homeBalance,carLoanRemaining:carBalance};
 return normalized
}
...
function initQaMode(){if(!QA_MODE)return;document.documentElement.classList.add('qa-mode');const banner=$('#qaBanner');if(banner)banner.hidden=false;$('#qaGenerate').onclick=()=>showDialog({title:'QA 35년 실사용 데이터 생성',message:'QA 저장소만 덮어씁니다. 운영 데이터와 Supabase에는 접근하지 않습니다.',confirmText:'생성',cancelText:'취소'},qaGenerateThirtyFiveYears);$('#qaMistakes').onclick=qaRunMistakes;$('#qaReset').onclick=()=>showDialog({title:'QA 데이터 초기화',message:'이 브라우저의 QA 데이터만 삭제합니다. 운영 데이터는 유지됩니다.',confirmText:'QA만 초기화',cancelText:'취소'},qaResetData);qaRenderStats()}
