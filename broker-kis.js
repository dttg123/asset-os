...
function brokerKisRightIncomeType(row){
 const event=brokerKisText(row?.rightTypeName||row?.eventType,100).toLowerCase();
 if(/상환|청산|감자|단주|합병|redemption|liquidation|capital_return/.test(event))return'other_right';
 if(/분배|distribution/.test(event))return'distribution';
 if(/배당|dividend/.test(event))return'dividend';
 if(/이자|interest/.test(event))return'interest';
 if(event)return'other_right';
...
function brokerKisIncomeCategory(type){return['dividend','distribution'].includes(type)?'dividend':type==='interest'?'interest':'other'}
function brokerKisRightClassificationBasis(row){
 const event=brokerKisText(row?.rightTypeName||row?.eventType,100).toLowerCase();
 if(event)return /상환|청산|감자|단주|합병|redemption|liquidation|capital_return|분배|distribution|배당|dividend|이자|interest/.test(event)?'event-description':'unverified';
 if(brokerKisRightIncomeType(row)==='other_right')return'unverified';
 return brokerKisText(row?.instrumentType||row?.productType,40)?'instrument-inferred':'name-inferred'
}
...
function brokerKisRightIncomeRecords(store,accountKind=''){
 const kind=brokerKisKind(accountKind);return(Array.isArray(store?.rights)?store.rights:[]).filter(row=>!kind||row.accountKind===kind).map(row=>{const type=brokerKisRightIncomeType(row);return{id:row.rightKey,accountId:row.accountId,accountKind:row.accountKind,holdingId:'',type,incomeCategory:brokerKisIncomeCategory(type),incomeType:type,date:row.cashPaymentDate||row.baseDate,baseDate:row.baseDate||'',amount:Math.max(0,Number(row.netAmount??((Number(row.amount)||0)-(Number(row.tax)||0)))||0),grossAmount:Math.max(0,Number(row.amount)||0),tax:Math.max(0,Number(row.tax)||0),label:row.productName||row.productCode||'한투 권리',productName:row.productName||'',source:'kis-right',readOnly:true,brokerRight:true,rightTypeCode:row.rightTypeCode||'',productCode:row.productCode||'',classification:row.classification||'unclassified_cash_right',rightTypeName:row.rightTypeName||'',classificationBasis:brokerKisRightClassificationBasis(row)}}).filter(row=>row.date&&row.amount>=0)
}
function brokerKisImportRights(store,rows,accountKind,accountId,fetchedAt=new Date().toISOString()){
 ...
 for(const raw of Array.isArray(rows)?rows:[]){const next=brokerKisNormalizeRight(raw,kind,accountId,timestamp);if(!next.accountId||!next.rightTypeCode||!next.productCode||(!next.baseDate&&!next.cashPaymentDate)){rejected++;continue}const current=byKey.get(next.rightKey);if(!current){list.push(next);byKey.set(next.rightKey,next);inserted++;continue}if(current.fetchedAt&&next.fetchedAt<=current.fetchedAt){skipped++;continue}// Partial responses must not erase previously supplied descriptions.
 for(const key of ['rightTypeName','instrumentType','productName'])if(!next[key])next[key]=current[key]||'';
 const metadataChanged=['rightTypeName','instrumentType','productName'].some(key=>(current[key]||'')!==next[key]);
