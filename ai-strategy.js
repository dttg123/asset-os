...
function aiPensionSection(kind){
 ...
 const income=aiRecentRows(pensionIncomeRecords(kind),240).map(row=>({date:row.date,type:row.type,amount:Number(row.amount)||0,grossAmount:Number(row.grossAmount??row.amount)||0,tax:Number(row.tax)||0,label:row.label||'',source:row.source||'',readOnly:!!row.readOnly,rightTypeCode:row.rightTypeCode||'',productCode:row.productCode||''}));
