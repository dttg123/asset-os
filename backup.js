...
async function parseBackupFile(file){const bytes=new Uint8Array(await file.arrayBuffer()),name=String(file.name||'').toLowerCase();if(name.endsWith('.json')||name.endsWith('.txt'))return JSON.parse(new TextDecoder().decode(bytes));return parseBackupZipBytes(bytes)}
...
function openBackupHub(){const size=new Blob([JSON.stringify(backupPayload())]).size,securePicker=typeof window.showSaveFilePicker==='function'&&window.isSecureContext,folderApi=typeof window.showDirectoryPicker==='function'&&window.isSecureContext,managed=typeof assetBackupSettings==='function'?assetBackupSettings():{phoneEnabled:false,lastPhoneBackupAt:'',phonePermission:'unknown'};$('#sheetEyebrow').textContent='데이터 보호';$('#sheetTitle').textContent='백업·복원';$('#sheetBody').innerHTML=`<div class=backup-grid><button class="backup-action primary" data-phone-folder><strong>휴대폰 백업 폴더 ${managed.phoneEnabled?'변경':'지정'}</strong><small>${folderApi?'한 번 지정하면 같은 폴더에 자동 저장합니다.':'이 브라우저는 고정 폴더를 지원하지 않아 일반 ZIP 저장을 사용합니다.'}</small></button><button class=backup-action data-phone-now ${managed.phoneEnabled?'':'disabled'}><strong>휴대폰 자동백업 지금 실행</strong><small>${managed.lastPhoneBackupAt?`마지막 ${formatDateTime(managed.lastPhoneBackupAt)}`:'아직 자동백업 없음'} · 같은 날짜 파일은 교체</small></button><button class=backup-action data-backup-zip><strong>ZIP 백업 · 저장 위치 선택</strong><small>${securePicker?'저장 위치를 직접 선택합니다.':'Downloads에 저장합니다.'}</small></button><button class=backup-action data-backup-drive><strong>Google Drive로 보내기</strong><small>공유 가능한 백업 TXT를 Drive로 보냅니다.</small></button><button class=backup-action data-backup-keep><strong>영구보관 ZIP</strong><small>자동 정리에서 제외되는 _keep 파일을 만듭니다.</small></button><button class=backup-action data-backup-restore><strong>ZIP / JSON / TXT 복원</strong><small>schema 4~${SCHEMA_VERSION} 파일을 검증하고 정상일 때만 교체합니다.</small></button></div><div class=backup-status-note>현재 원본 약 ${Math.max(1,Math.round(size/1024))}KB · schema ${SCHEMA_VERSION}<br>최근 30일은 일별, 이전은 월 1개만 남기며 _pre / _keep은 삭제하지 않습니다.</div>`;openSheet('#detailSheet');setTimeout(()=>{$('[data-phone-folder]').onclick=async()=>{if(await assetConnectPhoneBackup())openBackupHub()};$('[data-phone-now]').onclick=()=>assetWritePhoneBackup();$('[data-backup-zip]').onclick=saveZipBackup;$('[data-backup-drive]').onclick=shareDriveBackup;$('[data-backup-keep]').onclick=assetCreateKeepBackup;$('[data-backup-restore]').onclick=()=>$('#backupInput').click()},0)}
