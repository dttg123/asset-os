...
function pensionArchiveIncomeRecords(scope='all'){
 const kinds=new Set(pensionArchiveKinds(scope,'income'));return sourceArchiveRecords().filter(r=>kinds.has(r.kind)&&/^\d{4}-\d{2}/.test(String(r.date))).map(r=>({id:r.id,accountId:'',holdingId:'',accountKind:String(r.kind).startsWith('irp')?'irp':'pension',type:'dividend',date:r.date,amount:Math.max(0,Number(r.amount)||0),label:r.label,source:'source-archive'}))
}
